...
def base_redirect(request, error):
	if request.COOKIES['current'] == ('tob_view_users' or 'tower_of_bable' or 'tob_dics' or 'word_attributess' or 'mutawords' or 'complementary_scholar') :
		...
		response = redirect("Bable:"+request.COOKIES['current'], user=request.COOKIES['viewing_user'], dictionary=request.COOKIES['dictionary'], count=request.COOKIES['count'])
		response.set_cookie('error', error)
		return response
...
def tob_space_view(request, space):
	...
	user = User.objects.get(username=users_space.author.username)
	...
	if request.user.is_authenticated:
		...
		the_response = render(request, 'tob_space_view.html', {"users_sponsors": users_sponsors, "user_anon": user_anon, "users_space": users_space, "space_viewable": space_viewable, "spaces_posts": spaces_posts, 'loginform': loginform, 'registerform': registerform, 'registerterms': registerterms, 'posts_in_space': posts_in_space })
...
def tob_space_view_count(request, space, count):
	...
	particular_space = Space.objects.get(id=space)
	users_space = Space.objects.get(id=space)
	user_anon = Anon.objects.get(username=User.objects.get(username=users_space.author.username))
	spaces_posts = particular_space.posts.all()
	users_sponsors = users_space.sponsors.all()
	try:
		if spaces_posts is not None:
...
def tob_users_dic_word_attribute(request, user, dictionary, word, attribute):
	...
	if request.user.is_authenticated:
		the_response = render(request, "tob_users_dic_word_attributes.html", {"user_anon": user_anon, "loggedinanon": loggedinanon, "loggedinauthor": loggedinauthor, "dics_word": dics_word, "attribute_instance_id": int(attribute),
