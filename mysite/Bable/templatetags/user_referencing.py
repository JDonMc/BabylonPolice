...
def fontypes(value, words):
	...
	for word in words:
		if word.the_word_itself in value:
			...
			replace_to = '<div class="inner-{}-{} inner" style="cursor: pointer; width: fit-content; display: inline-block;"><a class="plain{} plain overlay" data-text="{}" style="color: yellow; font-style: url(\'{}\'); font-size: {}em; position: relative; display: inline-block;" href="{}">{}</a><style>{} p {{display: inline-block;}}</style><script>$("a.plain1").one("click", function() {{if( $(this).attr("href") > 0) {{$(this).attr("data", $(this).attr("href")); $(this).attr("href", "");$(this).addClass("active");$(".dropdown-menu-1").addClass("active");}} else {{$(this).attr("href", $(this).attr("data"));$(this).removeClass("active");$(".dropdown-menu-1").removeClass("active");}};$(".dropdown-menu-1").addClass("active");return false;}});</script></div>{}'.format(str(word.id), str(wordlen), word.id, word.the_word_itself, word.fontstyle, word.fontsize, reverse('Bable:tob_word', kwargs={'word_id':word.id}), word.the_word_itself, word.fontype, attribute_div1)
